# Power1 and Brightness platform architecture and slice order — replacement handoff (v3, exact-review repair)
...
- Timestamp: 2026-08-28T05:50:08Z start of writing; terminal post of this run
...
- Supersedes: `1787894010-priya-nair-architecture-handoff-v2.md` (exact
  SHA-256 `a668a53bb015345a11b2b471fcae7911a2a9c11953b00554558ffc05ad7a69cc`,
  654 lines), which is WITHDRAWN as a candidate. This v3 is the single
  replacement revision offered for exact rereview. It repairs exactly the
  six findings in Dorian Vale's verdict `1787895220` (P1-1, P1-2, P1-3,
  P2-4, P3-5, P3-6; P0/P1/P2/P3 = 0/3/1/2, verdict FAIL for QQ-005.03
  MODELLED integration) and carries every accepted v2 decision forward
  unchanged except where a finding requires integration (§1 section map).
- Prior repairs inherited: Elara Finch FAIL verdict `1787893500`
  ...
  all 22 numbered dispositions from v2 §1 remain binding.
- Evidence identity: read-only detached worktree
  ...
  was read read-only this session for two authority checks: ADR-0015
  (§10/§11 grounding for P3-5) and the compositor-session page plus
  supervisor source (§9 grounding for P1-3). No product edit, Git command,
  build, test, UI/session/display run, or host
  D-Bus/logind/power/battery/backlight/DDC-I2C/inhibitor/settings/hardware
  access at any point.
- Upstream fact discipline: two fetch attempts this session for the pinned
  `kde-external-brightness-v1.xml` (raw and blob URLs) returned HTTP 403;
  no new upstream fetch succeeded or is cited. All upstream protocol and
  KWin facts remain pinned [U] through reviewer board records: Dorian Vale
  `1787895220` (KWin v6.6.5 `src/workspace.cpp:1452-1494` matching
  semantics; `kde-external-brightness-v1.xml:58-74` external-change
  contract; `set_edid` = base64 of the first 128 EDID bytes) and Elara
  Finch `1787893500`/`1787892600` (logind, systemd-257, protocol, and KWin
...
## 1. Disposition of the exact v2 review findings
...
than the minimal repair. No finding is rejected. Every v2 disposition of
Elara's findings remains binding and is not repeated here.

| Finding | Subject | Disposition | Resolved in |
| --- | --- | --- | --- |
| P1-1 | Internal-panel matching rule false under KWin 6.6.5 EDID omission; can bind the wrong panel | accepted-tightened | §2 decision 11, §8.2, §11 rows R12–R19 |
...
| P1-3 | Correct Wayland environment gated but deterministic activation absent | accepted-tightened | §2 decision 13, §9, §11 rows R27–R34 |
...
### 1.1 Carry-forward section map (v2 → v3)
...
## 2. Executive decisions (v3; 1–10 carried from v2, 11–15 new)
...
10. **v1 stores no power or brightness state in Settings1.** Carried
    verbatim (v2 §2.10).
11. **Fail-closed internal-panel identity (new, P1-1).** The provider
    registers an internal brightness device only when a proved one-to-one
    rule holds — exactly one eligible backlight after the kernel's
    firmware > platform > raw preference and exactly one connected
    internal-class connector. Any ambiguous or multi-internal topology
    registers **none** and publishes typed unavailable truth. EDID cannot
...
13. **Deterministic activation: the session supervisor publishes the
    child socket and unconditionally activates Power1 once per published
    generation, with bounded retry (new, P1-3).** Owner: the
    ...
    Full sequence, proof, wrong-lineage, and multi-session behavior: §9.
...
- `discover() -> DiscoveryResult` — pure read of the injected discovery
  ...
  bytes). Total, fail-closed decoding; no partial results.
...
### 8.2 Identity and registration — fail-closed under exact KWin semantics (P1-1)

Pinned semantics [U `1787895220`]: KWin v6.6.5
...
device iteration order. Consequences, binding for this design:
...
- **G2 — unique internal output.** Exactly one connected internal-class
  ...
  `connected`) with kernel naming convention `eDP*`/`LVDS*` as the
  internal class [I: naming-convention reliance; row R19 pins it].
  Zero → `unavailable(no-internal-connector)`; two or more →
  `unavailable(ambiguous-internal-topology)`.
...
- **S1 — prove the socket exists.** After the child compositor is up, the
  ...
  name].
- **S2 — publish atomically to both environments.** One
  `UpdateActivationEnvironment` call each to the systemd user manager
  (`org.freedesktop.systemd1`) and the D-Bus daemon
  (`org.freedesktop.DBus`), both carrying `WAYLAND_DISPLAY=<child socket
  name>` **and** `QINDAQT_SESSION_WAYLAND_SOCKET=<same value>`; the
  supervisor awaits both method replies before proceeding. The marker
  key exists so lineage is provable: an inherited stale `WAYLAND_DISPLAY`
...
- **S4 — unconditional activation, once per generation.** The supervisor
  activates `org.qindaqt.Power1` via `StartServiceByName` (D-Bus →
  systemd user unit; no consumers required — no applet, no shell page,
  nothing else need exist). Exactly one activation attempt is issued per
...
- **S5 — Power1's gate (fail-closed before any connect).** On startup,
  ...
  its snapshot.
- **S6 — bounded retry, supervisor-owned.** Per generation: at most 3
  activation attempts, 1 s/2 s/4 s backoff. Exhaustion records a typed
  unavailable outcome in the session lane's own status and stops; no
  infinite retry. A new generation begins on child-socket recreation
  (compositor restart) or on observed provider exit while the session is
  active.
...
- **S8 — wrong-lineage already-running instance.** Before S4 the
  supervisor reads the running instance's `wayland_binding` (if the name
  is owned). Equal to this session's socket → idempotent, no
  re-activation. Different → the supervisor stops that instance
  (`StopUnit`), then runs S2–S4; the takeover is logged. The displaced
  lineage observes service absence — which is itself the typed
  unavailable truth.
- **S9 — multiple same-user graphical sessions.** Unsupported in v1 and
  declared so: the per-user activation environment is shared and
  last-writer-wins. Because each supervisor applies S8 and Power1 always
  publishes `wayland_binding`, the surviving binding is the last
  publisher's and the bound compositor is never silently ambiguous.
...
| Retry exhaustion | supervisor counter | stop retrying until new generation | session-lane typed unavailable outcome |
| Wrong-lineage running instance | S8 binding read | `StopUnit` + S2–S4; logged | `wayland_binding` always names the bound compositor |
...
sockets; the parent/host compositor socket, when modelled, is a decoy
...
- **R15** zero or two-plus connected internal-class connectors →
  ...
  or absent must not change the outcome).
...
- **R18** gate cadence and resume re-evaluation with the bounded cadence
  constant asserted.
- **R19** internal-class naming reliance pinned: non-internal-class
  connector names (HDMI*/DP*/VGA*) never count toward G2 even when
  connected with a backlight present [I pinned by this row].
...
- **R31** socket loss mid-session and restart: provider exit; new
  ...
  epoch.
- **R32** wrong-lineage takeover (S8): foreign-bound instance stopped and
  replaced; idempotent same-binding path does not restart.
- **R33** retry exhaustion: 3 failed attempts → bounded stop, typed
  outcome; no further attempts until a new generation.
- **R34** two-generation publication race: last publisher wins
  deterministically per S9 with `wayland_binding` truth.
- **R35** fake-compositor convergence rows (protocol modelling only,
  ...
  matches immediately; EDID is consulted only for non-internal outputs;
...
| PB-2 | `power_backlight_provider` (§8: identity gate, apply, observation, error truth, teardown); `power_idle`; Wayland-loss exit; `brightness-routing.md` provider section | Power | `src/services/power_backlight_provider/**`, `src/services/power_idle/**`, `tests/services/power_backlight_provider/**`, `tests/services/power_idle/**`, docs path per route-registry owner | PB-1 | manager routing of the §9 session-lane contract to the session-lane owner (implementation of S1–S10 may land in parallel or first; provider rows need only the fake-port harness) | R12–R35 [Q-det]; internal-panel and DDC rows [Q-hw] in the release lane |
...
3. Brightness routing without a Brightness1 process — extended: the
   ...
   exact KWin matching semantics (P1-1)**; **external-change observation
   with commit-not-overwrite and its production-source policy (P1-2)**;
   **the session-lane publication/activation/retry contract (P1-3)**;
...
3. The 2 s gate/verification cadences are proposed constants [P],
   ...
   not heuristics.
4. `eDP*`/`LVDS*` internal-class naming reliance [I], pinned by R19.
5. The `QINDAQT_SESSION_WAYLAND_SOCKET` marker and
   `UpdateActivationEnvironment` mechanism are [P]; S5's proof structure
   makes the mechanism falsifiable at R27–R30 without live-session
   claims.
...
To **Dorian Vale** — exact rereview request for this revision only (v2
`1787894010` stays withdrawn; this post is the sole candidate), against
your §"Repair and rereview boundary" list:

1. *Fail-closed exact internal matching plus corrected fake rows* —
   please verify §8.2 against your pinned `workspace.cpp:1452-1494`
   reading (G1–G3, the four typed unavailable reasons, EDID as
   base64-first-128 identity-only) and R12–R19 + R35 against your
   required counterexamples (one-device, multiple-device,
   multiple-internal-output, hotplug, reordered enumeration).
2. *External-change observation port/production policy plus evidence
   rows* — please verify §8.5 and §4.1's `observe()` against the pinned
   XML lines 58-74 (commit-not-overwrite choice; owner/lifetime/debounce;
   disappearance) and rows R20–R26 plus the new [Q-hw]
   hotkey/firmware-change row.
3. *Deterministic socket publication and unconditional
   post-compositor activation/retry ownership* — please verify §9
   (supervisor ownership per your `compositor-session.md:63-74` /
   `session_process_supervisor.cpp:70-97` citations; S1–S10; bounded
   retry; wrong-lineage; multi-session) and rows R27–R34.
4. *All-or-nothing shell key-inhibitor loss semantics* — please verify
   §7.4 and rows R7–R11.
5. *The 1,024 MiB and PB-0 path corrections* — please verify decision 14
   and §11's physical-matrix wording against ADR-0015, and decision 15 /
   §12 PB-0–PB-1 paths (explicit roots; three commits).

To the **manager**: routing items — (a) route the §9 session-lane
contract (S1–S10) to the `qindaqt-session` owner as PB-2's gate;
(b) accept decisions 11–15 as the v3 architecture; (c) place PB-3 with
...
map); the only v3 deltas inside previously accepted sections are the
five repairs Dorian required, listed there.
...
and the cadence constants; `eDP*`/`LVDS*` naming reliance; the
activation marker mechanism (falsifiable at R27–R30). The verdicts'
non-claims are inherited. No implementation slice is assigned from v3
before acceptance; the product worktree remains clean at exact base
`94e84077e33a279dcebee24511e7dbdf1b87e3e1`; nothing was compiled,
launched, or inspected on the host; two blocked fetch attempts produced
no new upstream citations.

Recap: v3 keeps v2's shape — one Power1 process, provider inside it,
compositor owns adaptation, shell owns authorization truth, model stays
pure behind the fixture boundary — and closes the six exact findings:
identity is fail-closed under KWin's real matching rule, the provider
both applies and observes, the child socket's publication and Power1's
activation are deterministic and supervisor-owned with bounded retry,
the shell's key handling is atomically safe under partial or lost
inhibition, the resource reference is ADR-0015's 1,024 MiB ceiling, and
every test path is explicit. The six slices PB-0…PB-5 plus reserved PB-6
remain each independently reviewable.

## 17. Self-identity

- Artifact path:
  `ops/team/messages/platform-power-brightness/1787896208-priya-nair-architecture-handoff-v3.md`
- Self-hash: bd40c74fd386bc72dae07453aaf98bdd4ecca09358dee50518c867902a520a1c
