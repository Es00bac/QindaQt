...
from qinda_icon_shapes import (
    APRICOT, BELL_CLAPPER_D, BELL_D, BLUE, BLUETOOTH_D, FOLDER_D,
    INK, JADE, PORCELAIN, VIOLET, Icon, dot, filled, gear_badge, ring, rect, stroke, strokes,
)
...
CANON = {
    ...
    "application-x-executable": Icon(
        ...
        rect(14, 10, 36, 44, rx=10, fill=JADE) + stroke("M26 24 38 32 26 40", color=PORCELAIN, width=6),
    ),
    "preferences-system": Icon(
        GROUP,
        ring(32, 32, 14) + strokes("M32 12v6", "M32 46v6", "M12 32h6", "M46 32h6"),
        f'<circle cx="32" cy="32" r="22" fill="{JADE}"/><circle cx="32" cy="32" r="9" fill="{PORCELAIN}"/>'
        + strokes("M32 6v8", "M32 50v8", "M6 32h8", "M50 32h8", color=JADE, width=6),
    ),
    "preferences-desktop-theme": Icon(
        GROUP,
        ring(24, 24, 8) + ring(40, 24, 8) + ring(32, 40, 8),
        dot(24, 24, 9, JADE) + dot(40, 24, 9, BLUE) + dot(32, 40, 9, APRICOT),
    ),
    "preferences-desktop-color": Icon(
        ...
        f'<path d="M32 10a22 22 0 0 1 0 44z" fill="{APRICOT}"/><path d="M32 10a22 22 0 0 0 0 44z" fill="{JADE}"/>'
    ...
    "notifications": Icon(
        ...
        filled(BELL_D, fill=JADE) + stroke(BELL_CLAPPER_D, color=INK, width=5),
    ),
    "clock": Icon(
        GROUP,
        ring(32, 32, 18) + stroke("M32 32V19M32 32 43 39"),
        f'<circle cx="32" cy="32" r="22" fill="{JADE}"/>'
    ...
    "system-file-manager": Icon(
        ...
        filled(FOLDER_D, fill=JADE) + stroke("M22 38h20", color=PORCELAIN, width=5),
    ),
    "accessories-text-editor": Icon(
        ...
        + stroke("M44 44 50 50", color=APRICOT, width=4),
    ),
    "utilities-terminal": Icon(
        GROUP,
        rect(10, 12, 44, 40, rx=8) + stroke("M20 26 28 34 20 42", width=5) + stroke("M36 42h10", width=5),
        rect(10, 12, 44, 40, rx=8, fill=INK) + stroke("M20 26 28 34 20 42", color=JADE, width=5)
        + stroke("M36 42h10", color=JADE, width=5),
    ),
}
