...
{
    return (value >= ushort('A') && value <= ushort('Z'))
        || (value >= ushort('a') && value <= ushort('z'));
}

bool isElementCharacter(ushort value) noexcept
{
    return isAsciiLetter(value)
        || (value >= ushort('0') && value <= ushort('9'))
        || value == ushort('_') || value == ushort('-');
}
...
bool validateWellKnownServiceName(const QString &serviceName, QString *error)
{
    ...
    if (serviceName.isEmpty() || serviceName.size() > 255) {
        return reject(QStringLiteral(
            "notification service name must contain 1 through 255 ASCII bytes"));
    }
    if (serviceName.startsWith(QLatin1Char(':'))) {
        return reject(QStringLiteral(
            "notification service name must be well-known, not a unique bus name"));
    }
    ...
    for (const QChar character : serviceName) {
        ...
        if (value == ushort('.')) {
            if (atElementStart) {
                return reject(QStringLiteral(
                    "notification service name contains an empty element"));
            }
            sawSeparator = true;
            atElementStart = true;
            continue;
        ...
        if (!isElementCharacter(value)) {
            return reject(QStringLiteral(
                "notification service name contains an invalid character"));
        }
        if (atElementStart && value >= ushort('0') && value <= ushort('9')) {
            return reject(QStringLiteral(
                "notification service name elements must not begin with a digit"));
        }
    ...
    if (!sawSeparator) {
        return reject(QStringLiteral(
            "notification service name must contain at least two elements"));
    }
    return true;
}
