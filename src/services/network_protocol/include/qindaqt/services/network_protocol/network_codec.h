...
// AGENT-CONTRACT: Canonical codecs are reentrant and borrow input only for
// the duration of a call. Decoders validate a complete temporary and replace
// the destination only on success, so malformed input can never partially
// replace a client's last accepted snapshot. Any byte-layout change requires
// a new accepted codec/protocol version.
