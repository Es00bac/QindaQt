    ...
    , m_registry(registry)
{
    ...
    m_chromePointerRouter = std::make_unique<HybridChromePointerRouter>(
        [this](const QPointF &position) {
            ...
            // ADR-0191: an iconified window's chip stacked above this
...
void KWinHybridSession::initializeChromeSceneLifecycle()
{
    ...
    m_chromeSceneLifecycle = std::make_unique<KWinChromeSceneLifecycle>(
        [this] {
            ...
            // Chip images are children of member WindowItems too (ADR-0191).
...
void KWinHybridSession::synchronizeChrome()
{
    ...
    for (const auto &containerId : m_runtime->topology().containerIds()) {
        ...
        if (options.shaded && options.containerTitle.isEmpty()) {
            ...
            // ADR-0168: the badge is told this name is generated, so it shows
            // the placeholder only when no page title can be shown instead.
