...
std::optional<ProcessCounters> parseProcess(const QString &procRoot, qint64 pid,
  ...
  // tail[] starts at stat field 3, so the parent pid is field 4 at
  // index 1. A process tree needs it; without a parent every row is a
  // root and the tree is just the flat list again.
  process.ppid = tail[1].toLongLong();
