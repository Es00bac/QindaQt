...
// Exercises: event.new dialog → create → on-disk persistence, occurrence
// model visibility, view.* action switching with coordinator checked-state
// sync, event.delete persistence, and fixture import. dataRoot is the
// controller's injected root; fixturesDir points at the committed .ics
// fixtures. Returns false with a diagnostic on the first failed step.
