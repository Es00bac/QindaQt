...
DesignTokens::TokenFacade *ensureOskTokenFacade(QQmlEngine &engine, QString *error)
{
    ...
    )qml", QUrl(QStringLiteral("inline:qindaqt-osk-token-registration.qml")));
    ...
    const std::unique_ptr<QObject> registrationObject(registration.create());
    ...
    auto *facade = engine.singletonInstance<DesignTokens::TokenFacade *>("QindaQt.Tokens", "Tokens");
...
bool applyOskAppearance(const AppAppearance::ApplicationAppearanceController &controller,
                        DesignTokens::TokenFacade &facade, QGuiApplication &application, QString *error)
{
