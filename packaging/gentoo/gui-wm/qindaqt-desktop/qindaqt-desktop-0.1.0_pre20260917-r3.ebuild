...
QINDAQT_COMMIT="b23b2ca31d6f97aeb9de7fd23c4399445e6859cf"
...
# This local, immutable checkpoint has not been published to the upstream remote.
...
# Streaming (O10). OBS is the streaming and recording application QindaQt
# drives over obs-websocket, so it is a runtime dependency of the desktop
# rather than something the user is expected to find and configure:
#   screencast - the PipeWire/portal capture OBS needs on Wayland
#   v4l        - the virtual camera output
#   wayland    - a native Wayland OBS, not an XWayland one
#   websocket  - the control surface the applet and Settings route speak to
# v4l2loopback provides the kernel device the virtual camera writes into;
# this package ships its modprobe.d and modules-load.d configuration.
RDEPEND="
	>=media-video/obs-studio-30[screencast,v4l,wayland,websocket]
