...
| `src/shell/launcher` | Separate targets: the pure bounded desktop-entry model (validation, deterministic category/search/ranking, pinned/recent identities, launch-intent presentation values) and the L1 runtime adapters (injected-root installed-application scanning with debounced watch and generation fencing, Settings1 dock/recent persistence (pins are the dock's applications, ADR-0265), seam-based bounded execution and D-Bus activation, and the shell-private applet controller plus compiled `QindaQt.Shell.Launcher` module) | Pure target: Qt Core only. Runtime target: pure target, public Settings1 client, the pure `src/services/dock_items` value, and Qt Core/DBus/Qml/Quick with filesystem, process, and bus reach confined to the scanner, spawner, and activator adapter files; production composition belongs to `src/shell/runtime`; never settings service internals or third-party process authority |
...
| `src/services/night_light` | Night light bounded values (modes, 1000–6500 K/100 K temperatures, coordinates, schedule times), the injected-path config port over the two owned files (`kwinrc [NightColor]`, `knighttimerc` General/Location/Times) with external-change truth, the live-state port over `org.kde.KWin.NightLight` with whole-frame validation and preview, and the `org.kde.NightTime` availability monitor | Qt Core, Qt DBus over injected paths/connections, and KConfig (`KF6::ConfigCore`) for owned-key IO; never KWin private ABI, the live compositor, solar computation, `knighttimestaterc`, Display1/ICC modules, Settings persistence, or QML |
...
| `src/shell/desktop_menu` | The shell-owned desktop menu shown while no application is active ([ADR-0260](../adr/0260-show-the-file-managers-menu-when-no-application-is-active.md)): the pure canonical-tree builder, presence/activation/confirmation control over the global-menu facade's desktop channel, the `DesktopMenuTargets` port, and the desktop-icons command channel | Public global-menu applet facade and protocol, File Manager's `qindaqt_file_manager_menu_catalog`, and Qt Core only; every effect leaves through the targets port that shell runtime implements over existing controllers. Never a service client, bus, process launcher, KWin, QML, the transport coordinator, or shell runtime |
...
| `src/session_supervisor` | Essential host/shell child startup, descriptor-only token handoff, shell-PID-authenticated Session1 logout, paced bounded shell recovery, optional restart-once secret-agent supervision, one-shot optional Welcome and XDG autostart process supervision after shell startup, parent-death-witnessed compositor-PID provisioning, and failure rollback | Public presentation-token protocol and session-autostart plans, Linux process identity/lifetime syscalls, and Qt Core/DBus; never Welcome preferences or UI, compositor internals, QML, login1, ScreenSaver, or service implementation libraries |
...
| `src/services/dock_items` | The dock's bounded value (`panels.dockItems`, ADR-0265): items, edits, uniqueness and bounds, the strict whole-value Settings1 codec, the one-time migration from `panels.launcherPinned`, and the confirmed-write `Settings1DockPins` helper other processes use to pin | Value target: Qt Core plus the launcher's pure bounds; never Settings1 transport, filesystem, process, D-Bus, or QML (paths are data). Pin target: the value plus the public Settings1 client; never the shell |
...
| `src/apps/file_manager` | Local directory listing/navigation, bounded file launch, identity-checked asynchronous local mutation, home Trash/recovery policy, read-only asynchronous smb/sftp network-location browsing behind an injected `NetworkDirectoryBackend` seam (S5, [ADR-0137](../adr/0137-file-manager-network-location-browsing.md)), AppShell actions/global-menu opt-in, the `go.applications` installed-application browser over the shared application catalog ([ADR-0164](../adr/0164-shared-application-catalog-and-file-manager-applications-browser.md)), stock `QtQuick.Controls` presentation, the `org.freedesktop.FileManager1` "Show in folder" service and Keep in Dock ([ADR-0273](../adr/0273-integrate-the-file-manager-with-the-desktop-like-finder.md)), and the narrow `public/` Desktop-consumer boundary (`FileBoundary`: listing, bounded launch, folder open and reveal in File Manager, mutation-controller composition, and clipboard-controller composition) | Public `QindaQt.AppShell 1.0` lifecycle/action seams and AppShell menu-export, `QindaQt::ApplicationCatalog` for the Applications browser, the dock's public `Settings1DockPins` helper over the public Settings1 client for Keep in Dock, plus Qt Core/Gui/Qml/Quick/QuickControls2/DBus (the FileManager1 adaptor), local POSIX filesystem calls, and (network module only) private `KF6::KIOCore`; presentation uses stock controls themed by the Qt platform theme per [ADR-0116](../adr/0116-build-bundled-applications-on-stock-qt6.md); never QST/Controls imports in app QML, shell internals, service internals, mount/per-volume-trash/portal authority, a remote credential store, or another app's private code |
| `src/session_autostart` | Read-only XDG autostart basename merge, condition evaluation, and bounded desktop-entry launch plans shared by Settings and session | Public ShellLauncher parser plus Qt Core; receives explicit roots/environment facts, never spawns, writes files, imports Settings UI, or owns the supervisor |
...
| `src/apps/settings/customize` | Layout preset gallery (ADR-0267): installed and user profile catalogs with provenance, confirmed Settings1 selection and auto-hide delay, own-preset save/rename/duplicate/delete and built-in restore through the profiles store, and responsive accessible QML; keeps `RepositoryCustomizeEditorHost` only as the live-host parity reference | Public Settings1 client, `profiles` (values, loader, user store), QST-1, QindaQt.Controls, QindaQt.Shell.Icons and Qt Core/QML (the reference host alone uses `shell_customization_editor`, `shell_customization` and applet manifests); never private repository headers, settings service persistence, shell surfaces, LayerShellQt, compositor/platform mutation, or layout editing |
...
| `src/apps/settings/input` | Input route models and QML over four injected ports: pointer/touchpad devices (KWin InputDevice D-Bus), keyboard repeat/NumLock (kcminputrc), keyboard layouts (kxkbrc plus a bounded evdev catalog), and global shortcuts (kglobalaccel listing, conflict naming, foreign assignment, command components) | KWin input and kglobalaccel D-Bus, KF6ConfigCore, Qt Core/DBus/Gui/QML/Quick, QST-1, and QindaQt.Controls; only the InputRouteComposition backend singleton constructs transports and config locations, QML/model code never touches D-Bus or files, and the module never writes schema-v2 settings, another route's model, or the live session outside the injected authorities (ADR-0134) |
...
| `src/apps/viewer` | Local image/PDF loading, bounded page rendering, view state and QindaTK presentation | Qt Core/Gui/QML/Quick, QindaTK, public AppShell/menu export, and privately linked Poppler Qt6; no Poppler types in QML or other modules, no shell internals, file mutation, network fetch, or new daemon ([ADR-0218](../adr/0218-use-qindatk-and-poppler-for-the-viewer.md)) |
| `src/apps/settings/default_apps` | Installed-application choices, ordered per-MIME XDG default lookup, mixed-category projection, and supported-scope association changes | Public ApplicationCatalog and local KConfig files; no subprocess, D-Bus, renderer, or application-private dependency. Packaged fallback policy lives in `data/session/qindaqt-mimeapps.list` ([Default applications](../apps/default-applications.md)) |
...
- The shell's desktop menu depends on File Manager only through the
  values-only `qindaqt_file_manager_menu_catalog` target rooted at
  `src/apps/file_manager/public` (menu titles, action labels, descriptions,
  shortcuts, and the application's title, entry id, and icon), which File
  Manager's own AppShell catalog also projects
  ([ADR-0260](../adr/0260-show-the-file-managers-menu-when-no-application-is-active.md)).
  It never links File Manager support code or the desktop file boundary.
  The desktop surface's menus read the same catalog for their words
...
- Desktop-scoped code depends only on the isolated
  ...
  open in QindaQt File Manager, the reveal that hands Get Info, Open With and
  New File to File Manager, and mutation-controller composition) and on the
  values-only menu catalog above. It never imports File Manager's
  `model/**`, `mutation/**`, or `app_shell/**` headers directly and never
  links the full File Manager support target; those remain File Manager's
  private controllers. See [File
