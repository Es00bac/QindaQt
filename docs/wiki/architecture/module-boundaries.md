...
| `src/apps/file_manager` | Local directory listing/navigation, bounded file launch, identity-checked asynchronous local mutation, home Trash/recovery policy, read-only asynchronous smb/sftp network-location browsing behind an injected `NetworkDirectoryBackend` seam (S5, [ADR-0137](../adr/0137-file-manager-network-location-browsing.md)), AppShell actions/global-menu opt-in, the `go.applications` installed-application browser over the shared application catalog ([ADR-0164](../adr/0164-shared-application-catalog-and-file-manager-applications-browser.md)), stock `QtQuick.Controls` presentation, and the narrow `public/` Desktop-consumer boundary (`FileBoundary`: listing, bounded launch, mutation-controller composition, and clipboard-controller composition) | Public `QindaQt.AppShell 1.0` lifecycle/action seams and AppShell menu-export, `QindaQt::ApplicationCatalog` for the Applications browser, plus Qt Core/Gui/Qml/Quick/QuickControls2, local POSIX filesystem calls, and (network module only) private `KF6::KIOCore`; presentation uses stock controls themed by the Qt platform theme per [ADR-0116](../adr/0116-build-bundled-applications-on-stock-qt6.md); never QST/Controls imports in app QML, shell internals, services, mount/per-volume-trash/portal authority, a remote credential store, or another app's private code |
