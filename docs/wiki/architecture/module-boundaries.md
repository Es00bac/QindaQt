...
| `src/app_shell/menu_export` | Opt-in composition of one AppShell action snapshot, one application window, and one injected session bus into the accepted canonical exporter, standard dbusmenu server, and platform-true AppMenu identity lifecycle | Public AppShell and global-menu protocol/exporter/dbusmenu/registrar boundaries plus Qt Core/DBus/Gui; the Qt 6.11 private KDE appmenu hook is confined to one Wayland identity adapter. Never app domain models, shell runtime/KWin, ambient bus lookup, menu-hosting inference, or action execution outside `ApplicationCoordinator` |
