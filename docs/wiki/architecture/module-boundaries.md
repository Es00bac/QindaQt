...
| `src/shell/task_list/producer` | Exact-owner asynchronous Compositor1 `Windows`/`Containers`/`ShellVisibilitySnapshot` reads, hostile complete-snapshot wire decoding, schema-2 fence coherence and scope-lineage regression fencing, collapsed-identity classification into T0 facts, and fail-closed Degraded publication with last-generation retention | Public task-list values and shell-visibility wire limits plus Qt Core/DBus in the injected-connection transport only; never KWin, shell runtime, QML, or a private compositor path |
| `src/shell/task_list/operations` | Serialized exactly-once Compositor1 `Submit`/`ReleaseContainer`/`DockWindows` adapter with owner/generation/container-lineage fencing, adapter-lifetime token lineage, canonical reply-lineage matching, and truthful Unavailable/Uncertain results | Public task-list producer lineage plus Qt Core/DBus; never window-level mutations the protocol does not expose, KWin, shell runtime, or QML |
