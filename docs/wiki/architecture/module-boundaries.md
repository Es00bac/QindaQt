...
| `src/services/notification_presentation_model` | Baseline/no-replay active, bounded popup, and in-memory recent projections; monotonic popup expiry; center state; and validated client operations | Public presentation client plus Qt Core; never Qt Quick/QML, LayerShellQt, host/service implementation, persistence, lock-screen, or do-not-disturb policy |
