...
| `src/shell_window_actions_client` | Exact-owner, one-in-flight authenticated compositor window-action transport with generation matching and uncertain no-replay outcomes | Public compositor action values plus Qt Core/DBus; never KWin objects, task-list/launcher policy, QML, or automatic retries |
...
| `src/compositor` | Persistence-neutral transaction bridges plus the release-matched KWin window registry, generation-retaining output inventory, authenticated panel-owner shell actions, development-only virtual-output adapter, topology scene adapter, ordinary chrome pointer router, member/transient policy, lifecycle synchronization, and D-Bus plugin | Public `core`/Hybrid/shell-visibility limits, Qt Core/DBus, and explicit KWin 6.6.5 extension points |
