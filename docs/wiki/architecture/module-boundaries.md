...
| `src/apps/settings/default_apps` | Installed-application choices, ordered XDG default lookup and category-scoped association changes | Public ApplicationCatalog and local KConfig files; no subprocess, D-Bus, renderer, or application-private dependency. Packaged fallback policy lives in `data/session/qindaqt-mimeapps.list` ([Default applications](../apps/default-applications.md)) |
