...
| `src/apps/file_manager` | Local directory listing/navigation, bounded file launch, identity-checked asynchronous local mutation, home Trash/recovery policy, AppShell actions/global-menu opt-in, stock `QtQuick.Controls` presentation, and the narrow `public/` Desktop-consumer boundary (`FileBoundary`: listing, bounded launch, and mutation-controller composition) | Public `QindaQt.AppShell 1.0` lifecycle/action seams and AppShell menu-export plus Qt Core/Gui/Qml/Quick/QuickControls2 and local POSIX filesystem calls; presentation uses stock controls themed by the Qt platform theme per [ADR-0116](../adr/0116-build-bundled-applications-on-stock-qt6.md); never QST/Controls imports in app QML, shell internals, services, mount/per-volume-trash/portal authority, or another app's private code |
