...
| `src/shell/global_menu` | Canonical bounded menu/action values, authenticated active-window provider ownership, fail-closed export lineage, the Qt Widgets menu adapter, and the shell-owned applet facade | Public protocol values plus Qt Core; Qt Gui/Widgets only inside the adapter target; never D-Bus transport, KWin objects, QML policy, or action execution |
