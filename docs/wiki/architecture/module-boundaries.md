...
| `src/apps/file_manager` | Local directory listing/navigation, bounded file launch, identity-checked asynchronous local mutation, home Trash/recovery policy, AppShell actions, and QST-1/Controls presentation | Public themes/QST-1/`QindaQt.Controls 1.0`/`QindaQt.AppShell 1.0` plus Qt Core/Gui/Qml/Quick/QuickControls2 and local POSIX filesystem calls; never shell internals, services, mount/per-volume-trash/portal authority, or another app's private code |
