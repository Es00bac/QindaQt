...
a refusal does not. Each call is capped at 2 s and resolution is budgeted at
3 s, checked before each call, so one resolution is bounded by the budget plus
one call and a logind that stalls every reply cannot block the resident service
for one timeout per examined session; a failed probe is then not repeated for
two seconds, because the sysfs source asks `available()` on every rescan.
...
| PB-4 | Display D7 class-B brightness policy and method: Display1 `GetBrightness` and `SetOutputBrightness` ([ADR-0150](../adr/0150-admit-immediate-external-output-brightness-through-display1.md)) | Accepted Display D2 |
