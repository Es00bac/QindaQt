...
that parent, and publishes the separately named `qindaqt-<run-id>` child socket
used by every QindaQt process.
...
16 deterministic sampled colors. The artifact, digest, byte count, dimensions,
and color diversity are archived before the private run root is removed.

The interaction probe sends exactly Meta press, N press, N release, and Meta
release to scenario-gated `Compositor1.InjectTestInput`. It requires the stable
development-device identity and then polls the compositor-owned shell-surface
...
1,048,576 KiB, PID/start-time authenticated teardown, durable failure archive,
