...
| Exact position | Position coordinate fields (X, Y) as an optional precision control | Accepts negative coordinates; validation normalizes the applied topology so its top-left display sits at 0,0. Meaningless for a mirrored output, which follows its source |
