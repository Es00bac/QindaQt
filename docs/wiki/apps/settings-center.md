...
modular settings routes. It contains six real routes: **Notifications**,
**Appearance**, **Display**, **Network**, **Customize**, and **Bluetooth**. The
shell owns route identity, selection, responsive presentation, and navigation
accessibility. Each route
...
the [Customize route](customize-settings.md); Bluetooth behavior is documented
on the [Bluetooth route](bluetooth-settings.md); notification quieting and its
live settings transaction remain documented under
...
Notifications, Appearance, Display, Network, Customize, or Bluetooth
...
`--page display`, `--page network`, `--page customize`, and `--page bluetooth`.
...
inactive host's six Loaders are all inactive. Switching layouts or routes
...
- Ctrl+1 through Ctrl+5 select Notifications, Appearance, Display, Network,
...
- construction starts all six route intents against an absent private bus and
...
- the installed row stages only `SettingsAppearanceRuntime`, removes host
  ...
  six routes, including the Customize catalogs and Bluetooth module, from only
...
service pages beyond the six compiled routes, search, arbitrary deep links,
