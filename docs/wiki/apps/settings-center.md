...
**Appearance**, **Display**, **Network**, **Customize**, and **Audio**. The shell owns route
...
the [Audio route](audio-settings.md); notification quieting and its live
...
Notifications, Appearance, Display, Network, Customize, or Audio component. It is not a
QML URL, plugin path, or service locator.

The public command accepts `--page notifications`, `--page appearance`,
`--page display`, `--page network`, `--page customize`, and `--page audio`.
...
inactive host's six route Loaders are all inactive. Switching layouts or routes
...
- the installed row stages only `SettingsAppearanceRuntime`, removes host
  ...
  reinstalls and proves all six routes, including the Customize module and
  catalogs, from only the complete relocated prefix; and
