...
views (ADR-0270), `qindaqt.file-manager-everyday-ui` for Quick Look,
type-to-select in every view and the Recents place (ADR-0272), and
`qindaqt.file-manager-styles-ui` for the Finder, Explorer and Commander styles
and tabs (ADR-0271). Fixtures remain below private temporary roots and use a
...
real hosted Task List applet, asserts `Tokens.ready`, the QindaPunk Nightfall
`bg.base`, a Controls label's concrete muted foreground, and atomic republish
to QindaPunk Porcelain. `qindaqt.shell-capture-matrix` now treats every QML
warning as fatal across the complete preview dispatcher and writes temporary
captures only beneath its build executable directory. The nested
...
| `qindaqt.network-networkmanager-visible-profile` | isolated fake NetworkManager capture of the exact Open/WPA-PSK/SAE settings map sent by the private libnm `AddAndActivateConnection` path, recursive secret-property absence, agent-owned PSK flags, and zero-call hidden/WEP/enterprise refusal |
...
The visible-profile row points the production libnm adapter at a minimal
fake NetworkManager on an isolated private system bus. That fake publishes
only one synthetic Wi-Fi device/access point and captures the exact
`AddAndActivateConnection` argument for assertions; it exposes no host device
or profile. The concrete owner probe and activation fixture likewise point
`DBUS_SYSTEM_BUS_ADDRESS` at isolated private brokers; one owns only the
NetworkManager bus name to exercise libnm owner notification. The production
adapter cannot observe or mutate the host. No
...
| `qindaqt.network-secret-agent-ip-config` | NetworkManager 1.56 DHCP profile shape plus every admitted Qt D-Bus wire form (`aa{sv}`, `a{sv}`, `aau`, `au`, `aay`, `a(ayuay)`, `a(ayuayu)`, `a{ss}`) over a real private-bus demarshal: bounded rows prompt, 257-item, oversized-key, and oversized-payload rows refuse without a prompt, a two-level wire form is admitted at depth 6 and refused at depth 7, and an unfamiliar signature (`ai`) fails closed |
| `qindaqt.network-secret-agent-lifetime` | the built `qindaqt-network-secret-agent` executable on the private bus: after its only prompt window closes it keeps running without `Unregister`, and a second `GetSecrets` prompts again |
...
and the QML/data roots confined to the stage. Finally it copies the stage twice,
...
The deterministic rows enforce the 32-source/128-lease registry, owner loss,
30-second hard expiry, canonical signed-64-bit Settings1 round trip, exact
phase/state/geometry envelope, and hostile identical-frame rejection without
opening a display or inherited bus.
...
first covers the left panel fullscreen. A setup Meta-drag restores it onto that
panel; the row then re-establishes hidden authority immediately before a second
private-seat Meta-drag. The causal drag must change compositor window geometry
and move the client clear of the panel. It makes the client fullscreen again,
captures that second hidden precondition, closes the client,
requires its compositor identity to disappear,
and captures the restored panel. Private pointer input
...
`shortcut-revealed`, `popup-held`, and `popup-closed`. Each PNG must have the
scenario's exact pixel dimensions, be a regular non-symlink file, pass checksum
decoding, and contain at least eight colors. The validator derives the stable
left/bottom panel rectangle from mapped/committed authority records, samples
that complete rectangle in every corresponding phase, and rejects equal region
digests across every hidden/visible pair. Eight copies of one valid unrelated
frame cannot qualify. Every phase also requires the top panel mapped and
committed with exclusive zone 30. Canonical evidence includes all eight
...
prove no fallback or silent retargeting. `qindaqt.audio-vban-operations`
replays a connected sender, retargets it, delivers delayed old link evidence,
disables/re-enables the same saved definition, and removes it; every transition
remains inactive until evidence carries the current activation token. It
never changes the user's live default sink. `qindaqt.settings-audio-page` exercises the compact 420×320
