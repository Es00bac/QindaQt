...
| Platform services | Audio, power, brightness, Bluetooth, network, clipboard, display/color/font settings, portals and policy | In progress (Audio1, Display D0-D4, resident Power PB-1, Bluetooth B0, resident Network N1 with confined NetworkManager transport, and the pure Display Color C0 boundary execute; Clipboard C0 and Font F0 foundations are WIRED; Network UI/credentials/hardware qualification, production BlueZ/UI/hardware, live color/font discovery and application, clipboard capture/service/UI, Display durable journal/resident composition/UI/nested convergence, production Power upstream adapters/policy/UI, hardware qualification, and other providers remain) |
