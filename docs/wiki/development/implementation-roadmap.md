...
| Platform services | Audio, power, brightness, Bluetooth, network, clipboard, display/color/font settings, portals and policy | In progress (Audio1, Display D0-D2, Bluetooth B0, and Network N0 execute; PB-0, Clipboard C0, and Font F0 pure foundations are WIRED; resident Network1/NetworkManager transport/UI, production BlueZ/UI/hardware, live font discovery/application, clipboard capture/service/UI, Display client/writer/UI, resident Power service/client/upstream adapters, hardware qualification, and other providers remain) |
| First-party experience | Settings center and core applications with accessibility and consistent theming | In progress (QST-1 and reusable QindaQt.Controls are independently qualified; Notifications and Appearance are live installed Settings routes; Text Editor S1, bounded local File Manager S0, single-session Terminal S0, and the narrow installed QindaQt.AppShell 1.0 action/lifecycle/portal/focus/accessibility boundary are executable; remaining service routes, complete Settings navigation, later File Manager and Terminal capabilities, app migrations, cross-app and nested-session matrices, and live accessibility bridge pending) |
