...
| Platform services | Audio, power, brightness, Bluetooth, network, clipboard, display/color/font settings, portals and policy | In progress (Audio1, Display D0-D3, Bluetooth B0, Network N0, and the pure Display Color C0 boundary execute; PB-0, Clipboard C0, and Font F0 foundations are WIRED; resident Network1/NetworkManager transport/UI, production BlueZ/UI/hardware, live color/font discovery and application, clipboard capture/service/UI, Display writer/UI, resident Power upstream adapters/UI, hardware qualification, and other providers remain) |
