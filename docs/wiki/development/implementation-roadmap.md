...
| Platform services | Audio, power, brightness, Bluetooth, network, clipboard, display/color/font settings, portals and policy | In progress (Audio1, Display D0-D5, resident Power PB-1 plus its production shell consumer, Bluetooth B0, Network N0, and the pure Display Color C0 boundary execute; Clipboard C0 and Font F0 foundations are WIRED; resident Network1/NetworkManager transport/UI, production BlueZ/UI/hardware, live color/font discovery and application, clipboard capture/service/UI, Display resident startup recovery/writer composition/nested convergence, production Power upstream adapters and hardware qualification, and other providers remain) |
