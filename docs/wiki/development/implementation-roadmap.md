...
| Platform services | Audio, power, brightness, Bluetooth, network, clipboard, display/color/font settings, portals and policy | In progress (Audio1 typed stack and isolated null-device proof plus revisioned compositor output inventory and contained virtual-output seam implemented; Display1 transactions, hardware/UI qualification, and other providers pending) |
