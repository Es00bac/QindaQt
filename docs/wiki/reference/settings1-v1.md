...
`panels.launcherRecent`, each a string list with an empty default, and the
dock value `panels.dockItems`, an object with the default `{}`
([ADR-0265](../adr/0265-keep-dock-items-in-one-structured-settings-value.md)).
All three keys must be included in its client's snapshot scope, which may also
contain other shell settings. Since ADR-0265 the pins are the dock's
applications; `panels.launcherPinned` is read only until the first dock edit
and never written. Desktop-entry identity validation, uniqueness and the
bounds of 16 legacy pins and 8 recent entries remain launcher-owned; the dock
value's shape and bounds belong to `QindaQt::Services::DockItems`, which other
processes use through `Settings1DockPins` with the scope `panels.dockItems`
plus `panels.launcherPinned`. Schema-normalized string lists persist as JSON
arrays and regain their string-list type when loaded.
