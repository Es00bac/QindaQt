...
| `qindaqt.clipboard-applet-consent-{missing,malformed,inherited-default,ownerless}` | Registered private-bus negative controls: every non-explicit consent kind withholds pre-existing Clipboard1 content immediately, remains unavailable while the old content-bearing generation is authoritative, and accepts Disabled only from the generation-advanced empty purge snapshot. |
