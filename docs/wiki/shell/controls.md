...
| `FormRow` | Responsive label/helper/error plus editor host. `editor` is required; the row seats the assigned editor in its editor host, so call sites may declare the editor inside the row or assign it inline (`editor: Slider {}`) — both land in the same host. Size inline editors with a plain `width:`, not `Layout` attached properties, because the host is a plain Item. While associated, the row's required label and current helper/error description intentionally supersede the editor's own `accessibleName` and `accessibleDescription`; its native role and value interface remain intact. |
