...
on the new exact-name `QScreen`. An unchanged semantic primary reuses the
existing windows when no invalidation-withdrawal turn intervenes; if the
authoritative reread completes later, fail-closed clearing may briefly recreate
the pair before the same coherent route returns. Owner loss,
removal/replacement races, generation mismatch, or a missing exact Qt screen
clears both windows until a coherent frame returns. Panel and dock
reconciliation keeps its independent safe-visible policy.
