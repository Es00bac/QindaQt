...
### QQ-006.08 — Terminal (QQ_Term)

**State:** `EXECUTABLE`. **Weight:** 8.

The desktop's terminal is QQ_Term (`gui-apps/qqterm`), built in the QindaQt_Apps repository on `qtermwidget`: one session per window with profiles, 42 colour schemes, per-window theme rotation, per-profile transparency, global-menu export, and single-instance hand-off; the desktop's window containers supply tabs and splits. The 8100-line `qindaqt-terminal` built in this repository was removed in favour of it (ADR-0222), so the desktop ships one terminal, one desktop entry, one icon, and one launcher row. `Terminal=true` desktop entries launch for the first time: the shell wires `{"qqterm", "-e"}` as the launcher's terminal command prefix, where the prefix had been empty and every such entry refused.

**Caveat:** the terminal's own behaviour is no longer pinned by this repository's suite. Six integration values (executable name, desktop entry, `StartupWMClass`, icon name, bus name, and the `-e` form) are a cross-repository contract recorded on the owning page; GPU rendering, whole-application assistive-technology proof, the nested screenshot matrix, and physical display/input qualification remain.
...
- **test** — [terminal](../../apps/terminal.md): QQ_Term replaces qindaqt-terminal on both machines: `src/apps/terminal` and `tests/apps/terminal` are removed with the `Terminal` install component, release-contract entry, desktop entry and icon alias; `gui-wm/qindaqt-desktop` gains the `gui-apps/qqterm` runtime dependency; the desktop context menu, icon theme, Gabbee private lanes and the `Terminal=true` launch policy all resolve `org.qindaqt.QQTerm`.
