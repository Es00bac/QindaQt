...
- Voice (`6a2d9e2d`, `58ca80c7`): confirmed Off withdraws desktop provider
  ...
  9/9 focused/route/private-bus rows and broad Settings suite (123/123)
  pass. Physical microphone and nested-shell behavior remain to qualify.
- Startup (`1d5de5e3`, `79a037c8`): Settings and the session share XDG
  autostart eligibility; eligible entries run once per login under session
  ownership, and private-session tests suppress ambient user autostarts.
...
bundled applications; [ADR-0116](wiki/adr/0116-build-bundled-applications-on-stock-qt6.md)
...
Qt 6 themed through the Qt platform theme (ADR-0115). All four apps are
converted, their listed features landed, and every app's focused rows pass at
the integrated tree (Calendar 12/12, File Manager 31/31, Text Editor 27/27,
Terminal 24/24). Remaining limits and the next packaging step (re-pin the
desktop ebuild at or after `dc992d63` and rebuild through Portage) are in
[Handoff](HANDOFF.md). Per-app detail follows for the record.
...
**Browsing slice integrated and installed through Portage r1.** Draggable scrollbars,
...
capture rows pass. The installed payload, native app smoke, and broader desktop failures are recorded
