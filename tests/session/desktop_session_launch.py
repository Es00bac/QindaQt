...
def _traced_session_program(
    ...
    wrapper.write_text(
        ...
        "--no-polkit-agent --no-powerdevil --no-global-shortcut-daemon --no-autostart\n",
        encoding="utf-8",
    )
    wrapper.chmod(0o700)
    return wrapper
...
def _session_program(
    ...
    wrapper.write_text(
        ...
        f"--theme {shlex.quote(scenario.theme_id)} --no-polkit-agent --no-powerdevil --no-global-shortcut-daemon --no-autostart\n",
        encoding="utf-8",
    )
    wrapper.chmod(0o700)
    return wrapper
