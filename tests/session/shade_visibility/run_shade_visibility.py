...
AGENT-GUARD: never point this at the live desktop. Input comes only from
the private compositor's development device, never from host uinput, and every
HOME/XDG root is this row's own.
...
(/tmp where there is none) and are removed when the row ends. Evidence and
captures still land under --output-root. See private_bus_root.
"""
...
def fresh_runtime_root(output_root: Path) -> Path:
    base = private_bus_root(output_root) / "sv"
...
def main() -> int:
    ...
    # The runtime tree is outside --output-root now, so this row owns
    # taking it away again rather than leaving one per run in the
