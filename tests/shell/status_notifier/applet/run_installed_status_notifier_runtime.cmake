...
foreach(required IN ITEMS QINDAQT_CMAKE QINDAQT_BUILD_ROOT QINDAQT_STAGE
    ...
    if(NOT DEFINED ${required} OR "${${required}}" STREQUAL "")
        message(FATAL_ERROR "Missing installed Status Notifier runtime input: ${required}")
    endif()
endforeach()
...
if(NOT stage_is_in_build OR stage STREQUAL build_root)
    message(FATAL_ERROR "Status Notifier runtime stage must be a child of the build tree")
endif()

file(REMOVE_RECURSE "${stage}")
execute_process(
    COMMAND "${QINDAQT_CMAKE}" --install "${build_root}"
            --prefix "${stage}" --component StatusNotifierAppletRuntime
    RESULT_VARIABLE install_status
    OUTPUT_VARIABLE install_output
    ERROR_VARIABLE install_error)
if(NOT install_status EQUAL 0)
    message(FATAL_ERROR
        "StatusNotifierAppletRuntime install failed:\n${install_output}${install_error}")
endif()

set(shell "${stage}/${QINDAQT_INSTALL_BINDIR}/qindaqt-shell")
set(data "${stage}/${QINDAQT_INSTALL_DATADIR}/qindaqt")
set(module "${stage}/${QINDAQT_INSTALL_QMLDIR}/QindaQt/Shell/StatusNotifier")
foreach(required_path IN ITEMS
        ...
        "${module}/qmldir"
        "${module}/qindaqt_shell_status_notifier_applet.qmltypes"
        "${module}/qml/StatusNotifierApplet.qml"
        "${module}/qml/StatusNotifierItemDelegate.qml"
        "${module}/libqindaqt_shell_status_notifier_applet_runtime.a"
        "${module}/libqindaqt_shell_status_notifier_applet_runtimeplugin.a"
    ...
    if(NOT EXISTS "${required_path}")
        message(FATAL_ERROR
            "StatusNotifierAppletRuntime stage is missing ${required_path}")
    endif()
endforeach()

file(STRINGS "${shell}" compiled_boundary
     REGEX "(StatusNotifierApplet|org.kde.StatusNotifierWatcher|status-items.read)")
if(NOT compiled_boundary)
    message(FATAL_ERROR
        "Staged shell contains no compiled Status Notifier composition evidence")
endif()
...
if(NOT list_status EQUAL 0)
    message(FATAL_ERROR
        "Staged Status Notifier shell failed under source poison:\n"
        "${list_output}${list_error}")
endif()
string(FIND "${list_output}" "status-notifier - Status Notifier" status_notifier_entry)
if(status_notifier_entry EQUAL -1)
    message(FATAL_ERROR
        "Staged shell did not resolve Status Notifier:\n${list_output}")
endif()

message(STATUS
    "Installed Status Notifier production shell, compiled module, and source-poison proof passed")
