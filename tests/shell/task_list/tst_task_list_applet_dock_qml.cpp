...
void TaskListAppletDockQmlTests::magnifiedTileGrowsAboveTheShelfInsideTheReservedEnvelope()
{
  TaskListSource source;
  FakeOperationAuthority authority;
  FakeTaskListOperationPort port;
  ...
  QVERIFY(zoomSurface->y() + zoomSurface->height() <= dockEntry(root)->y() + 0.01);
  ...
  // and the delegate bounds never move.
  root->setProperty("dockPointerX", first->x() + first->width() / 2);
  // The falloff binding peaks exactly under the strip-local pointer. The
  // rendered scale is Behavior-driven and the offscreen platform may drop a
  // stray synthetic hover into the envelope band (the same tracking surface
  // under test), so the rendered assertion is bounded, not exact.
  QCOMPARE(first->property("dockZoomScale").toDouble(), 1.5);
  QTRY_VERIFY(firstIcon->property("scale").toDouble() > 1.4);
  const qreal renderedScale = firstIcon->property("scale").toDouble();
  QVERIFY(renderedScale <= 1.5);
