...
bool isExactServiceProcess(const pid_t pid)
{
    const QString expected =
        QFileInfo(QStringLiteral(QINDAQT_AUDIO_SERVICE_EXECUTABLE)).canonicalFilePath();
...
public:
    bool start(const bool reserveUniqueOwner = false)
    {
        ...
        QFile descriptor(serviceDir + QStringLiteral("/org.qindaqt.Audio1.service"));
        if (!descriptor.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
            return false;
        }
        descriptor.write("[D-BUS Service]\nName=org.qindaqt.Audio1\nExec=");
        descriptor.write(QByteArray(QINDAQT_AUDIO_SERVICE_EXECUTABLE));
        descriptor.write("\n");
        descriptor.close();
        ...
        environment.insert(QStringLiteral("PIPEWIRE_RUNTIME_DIR"), runtimeDir);
        ...
        connectionName = QStringLiteral("qindaqt-audio-activation-%1")
    ...
    QTemporaryDir root{QStringLiteral("/tmp/qindaqt-audio-activation-XXXXXX")};
    QProcess daemon;
    QString address;
...
class AudioActivationTests final : public QObject
{
    Q_OBJECT
...
void AudioActivationTests::daemonLossExitsAndReplacementStartsFresh()
{
    registerDBusTypes();
...
QTEST_GUILESS_MAIN(AudioActivationTests)
#include "tst_audio_activation.moc"
