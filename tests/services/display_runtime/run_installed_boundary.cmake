...
foreach(required IN ITEMS QINDAQT_CMAKE QINDAQT_BUILD_DIRECTORY
    ...
    if(NOT DEFINED ${required})
        message(FATAL_ERROR "Missing installed Display runtime input: ${required}")
    endif()
endforeach()
...
if(NOT prefix_is_in_build OR install_prefix STREQUAL build_directory)
    message(FATAL_ERROR "Refusing to replace a Display runtime stage outside its build")
endif()

file(REMOVE_RECURSE "${install_prefix}")
set(install_command
    "${QINDAQT_CMAKE}" --install "${build_directory}"
    --prefix "${install_prefix}" --component DisplayRuntimeDevelopment
)
...
if(NOT install_status EQUAL 0)
    message(FATAL_ERROR
        "Staged Display runtime install failed:\n${install_output}${install_error}")
endif()
...
if(NOT check_status EQUAL 0)
    message(FATAL_ERROR
        "Installed Display runtime boundary failed:\n${check_output}${check_error}")
endif()

set(consumer_build "${install_prefix}/consumer-build")
execute_process(
    COMMAND
        "${QINDAQT_CMAKE}" -S "${QINDAQT_CONSUMER_SOURCE_DIRECTORY}"
...
if(NOT configure_status EQUAL 0)
    message(FATAL_ERROR
        "Installed runtime consumer configure failed:\n${configure_output}${configure_error}")
endif()
execute_process(
...
if(NOT build_status EQUAL 0)
    message(FATAL_ERROR
        "Installed runtime consumer build failed:\n${build_output}${build_error}")
endif()

file(WRITE
    "${stage_include}/qindaqt/services/display_runtime/private_session_adapter.h"
...
if(poison_status EQUAL 0)
    message(FATAL_ERROR
        "Installed checker accepted a planted private runtime header:\n"
        "${poison_output}${poison_error}")
endif()

file(REMOVE_RECURSE "${install_prefix}")
message(STATUS "Installed Display runtime public boundary and poison verified")
