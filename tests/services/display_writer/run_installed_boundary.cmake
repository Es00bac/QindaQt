...
foreach(required IN ITEMS QINDAQT_CMAKE QINDAQT_BUILD_DIRECTORY
                          ...
                          QINDAQT_CONSUMER_SOURCE QINDAQT_QT6_DIR
                          QINDAQT_GENERATOR CHECK_SCRIPT)
    if(NOT DEFINED ${required})
        message(FATAL_ERROR "Missing installed Display writer input: ${required}")
    endif()
endforeach()
...
if(NOT prefix_is_in_build OR install_prefix STREQUAL build_directory)
    message(FATAL_ERROR "Refusing to replace a Display writer stage outside its build")
endif()

file(REMOVE_RECURSE "${install_prefix}")
set(install_command
    "${QINDAQT_CMAKE}" --install "${build_directory}"
    --prefix "${install_prefix}" --component DisplayWriterDevelopment
)
...
if(NOT install_status EQUAL 0)
    message(FATAL_ERROR
        "Staged Display writer install failed:\n${install_output}${install_error}")
endif()
...
if(NOT check_status EQUAL 0)
    message(FATAL_ERROR
        "Installed Display writer boundary failed:\n${check_output}${check_error}")
endif()
...
if(NOT configure_status EQUAL 0)
    message(FATAL_ERROR
        "Installed header consumer configure failed:\n${configure_output}${configure_error}")
endif()
execute_process(
...
if(NOT build_status EQUAL 0)
    message(FATAL_ERROR
        "Installed header consumer build failed:\n${build_output}${build_error}")
endif()

# Poison negative: a private protocol-facing header in the installed namespace
# must make the same package checker fail. The stage is disposable and guarded.
file(WRITE
    "${stage_include}/qindaqt/services/display_writer/private_protocol_adapter.h"
...
if(poison_status EQUAL 0)
    message(FATAL_ERROR
        "Installed package checker accepted a planted private writer header:\n"
        "${poison_output}${poison_error}")
endif()

message(STATUS "Installed Display writer public/private boundary and poison verified")
file(REMOVE_RECURSE "${install_prefix}")
