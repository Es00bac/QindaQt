...
void AppearanceSettingsModelAdversarialTests::ownerReplacementBetweenReplyAndSnapshotDoesNotReplay()
{
    auto *model = makeModel();
    QVERIFY(model != nullptr);
    ...
    m_owner = QStringLiteral(":1.99");
    Q_EMIT m_transport.ownerChanged(m_owner);
    QVERIFY(answerSnapshot(m_transport, QStringLiteral("epoch-b"), 0,
                           baselineValues()));
    QTRY_VERIFY(model->ready());
    QCOMPARE(m_transport.commits.size(), 1);
    QVERIFY(model->draftDirty());
...
void AppearanceSettingsModelAdversarialTests::confirmedFailureDiagnosticSurvivesRebaseline()
{
    auto *model = makeModel();
    QVERIFY(model != nullptr);
    ...
    QVERIFY(answerSnapshot(m_transport, QStringLiteral("epoch-a"), 7,
                           baselineValues()));
    QTRY_VERIFY(model->ready());
    QCOMPARE(model->errorText(), QStringLiteral("durable save failed"));
    QCOMPARE(m_transport.commits.size(), 1);
...
void AppearanceSettingsModelAdversarialTests::invalidSnapshotTypeFailsClosed()
{
    AdversarialTransport transport;
    ...
    Q_EMIT transport.ownerChanged(QStringLiteral(":1.45"));
    QVERIFY(answerSnapshot(transport, QStringLiteral("epoch-c"), 2,
                           baselineValues()));
    QTRY_VERIFY(model.ready());
}
