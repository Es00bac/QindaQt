...
foreach(required IN ITEMS BUILD_DIRECTORY INSTALL_PREFIX INSTALL_BINDIR
    ...
    if(NOT DEFINED ${required})
        message(FATAL_ERROR "missing installed Power route input: ${required}")
    endif()
endforeach()
...
if(NOT in_build OR install_prefix STREQUAL build_directory)
    message(FATAL_ERROR "Power route stage must remain below its build tree")
endif()
...
if(NOT install_status EQUAL 0)
    message(FATAL_ERROR "Power route install failed:\n${install_output}${install_error}")
endif()

set(executable "${install_prefix}/${INSTALL_BINDIR}/${SETTINGS_EXECUTABLE_NAME}")
set(module "${install_prefix}/${INSTALL_QMLDIR}/QindaQt/SettingsApp/Power")
...
foreach(required_path IN ITEMS "${executable}" "${module}" "${theme_directory}")
    if(NOT EXISTS "${required_path}")
        message(FATAL_ERROR "installed Power route payload missing: ${required_path}")
    endif()
endforeach()
set(developer_module "${build_directory}/qml/QindaQt/SettingsApp/Power")
...
if(NOT poison_status EQUAL 3)
    message(FATAL_ERROR
        "relocated Settings borrowed an uninstalled Power module; status ${poison_status}:\n${poison_status_log}")
endif()
...
if(NOT reinstall_status EQUAL 0)
    message(FATAL_ERROR "Power route reinstall failed:\n${reinstall_output}${reinstall_error}")
endif()
run_route(route_status)
if(NOT route_status MATCHES "[Tt]imeout")
    message(FATAL_ERROR
        "relocated Power route did not remain constructed (${route_status}):\n${route_status_log}")
endif()
message(STATUS "Relocated Power Settings route remained resident with host buses poisoned")
