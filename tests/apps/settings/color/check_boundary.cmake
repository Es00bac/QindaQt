...
elseif(DEFINED SOURCE_ROOT)
    get_filename_component(route_root "${SOURCE_ROOT}/src/apps/settings/color" ABSOLUTE)
else()
...
foreach(source IN LISTS route_files)
    ...
    if((contents MATCHES "QtDBus|QDBus") AND
       NOT source MATCHES "/color_route_composition.cpp$")
        message(FATAL_ERROR "Color Settings model or page imported direct D-Bus in ${source}")
    endif()
    ...
    foreach(include_directive IN LISTS include_directives)
        ...
        if(NOT include_allowed)
            message(FATAL_ERROR "Color Settings imported a non-public repository header '${include_path}' in ${source}")
        endif()
    endforeach()
    if(contents MATCHES "Q_INVOKABLE[^\n]*(applyToCompositor|applyProfile|setCompositor|setOutputColor)")
        message(FATAL_ERROR "Color Settings gained forbidden compositor-application authority in ${source}")
    endif()
endforeach()
...
if(file_count EQUAL 0)
    message(FATAL_ERROR "Color Settings boundary found no route files")
endif()
message(STATUS "Color Settings public-client allow-list accepted ${file_count} files")
