...
foreach(required IN ITEMS BUILD_DIRECTORY INSTALL_PREFIX INSTALL_BINDIR
    ...
    if(NOT DEFINED ${required})
        message(FATAL_ERROR "missing installed Color route input: ${required}")
    endif()
endforeach()
...
if(NOT in_build OR install_prefix STREQUAL build_directory)
    message(FATAL_ERROR "Color route stage must remain below its build tree")
endif()
...
if(NOT install_status EQUAL 0)
    message(FATAL_ERROR "Color route install failed:\n${install_output}${install_error}")
endif()

set(executable "${install_prefix}/${INSTALL_BINDIR}/${SETTINGS_EXECUTABLE_NAME}")
set(module "${install_prefix}/${INSTALL_QMLDIR}/QindaQt/SettingsApp/Color")
...
foreach(required_path IN ITEMS "${executable}" "${module}" "${theme_directory}")
    if(NOT EXISTS "${required_path}")
        message(FATAL_ERROR "installed Color route payload missing: ${required_path}")
    endif()
endforeach()
set(developer_module "${build_directory}/qml/QindaQt/SettingsApp/Color")
if(NOT IS_DIRECTORY "${developer_module}")
    message(FATAL_ERROR "Color relocation poison needs the developer module present")
endif()

set(sandbox "${install_prefix}/color-route-runtime")
...
if(NOT poison_status EQUAL 3)
    message(FATAL_ERROR
        "relocated Settings borrowed an uninstalled Color module; status ${poison_status}:\n${poison_status_log}")
endif()
...
if(NOT reinstall_status EQUAL 0)
    message(FATAL_ERROR "Color route reinstall failed:\n${reinstall_output}${reinstall_error}")
endif()
