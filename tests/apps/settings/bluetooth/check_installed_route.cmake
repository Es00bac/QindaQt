...
foreach(required IN ITEMS BUILD_DIRECTORY INSTALL_PREFIX INSTALL_BINDIR
                          ...
                          SETTINGS_EXECUTABLE_NAME)
    if(NOT DEFINED ${required})
        message(FATAL_ERROR "missing installed Bluetooth route input: ${required}")
    endif()
endforeach()
...
if(NOT in_build OR install_prefix STREQUAL build_directory)
    message(FATAL_ERROR "Bluetooth route stage must remain below its build tree")
endif()
...
if(NOT install_status EQUAL 0)
    message(FATAL_ERROR "Bluetooth route install failed:\n${install_output}${install_error}")
endif()

set(executable "${install_prefix}/${INSTALL_BINDIR}/${SETTINGS_EXECUTABLE_NAME}")
set(module "${install_prefix}/${INSTALL_QMLDIR}/QindaQt/SettingsApp/Bluetooth")
...
foreach(required_path IN ITEMS "${executable}" "${module}" "${theme_directory}")
    if(NOT EXISTS "${required_path}")
        message(FATAL_ERROR "installed Bluetooth route payload missing: ${required_path}")
    endif()
endforeach()
set(developer_module "${build_directory}/qml/QindaQt/SettingsApp/Bluetooth")
if(NOT IS_DIRECTORY "${developer_module}")
    message(FATAL_ERROR "Bluetooth relocation poison needs the developer module present")
endif()

set(sandbox "${install_prefix}/bluetooth-route-runtime")
file(MAKE_DIRECTORY "${sandbox}/config" "${sandbox}/data"
                    "${sandbox}/system-data" "${sandbox}/cache"
                    "${sandbox}/runtime")
...
function(run_route result_name)
    execute_process(
        COMMAND "${CMAKE_COMMAND}" -E env
                ...
                XDG_DATA_DIRS=${sandbox}/system-data
                XDG_CACHE_HOME=${sandbox}/cache
                XDG_RUNTIME_DIR=${sandbox}/runtime
                "${executable}" --page bluetooth
...
set(withheld "${module}.withheld")
file(RENAME "${module}" "${withheld}")
run_route(poison_status)
file(RENAME "${withheld}" "${module}")
if(NOT poison_status EQUAL 3)
    message(FATAL_ERROR
        "relocated Settings borrowed an uninstalled Bluetooth module; status ${poison_status}:\n${poison_status_log}")
endif()

execute_process(COMMAND ${install_command} RESULT_VARIABLE reinstall_status
                OUTPUT_VARIABLE reinstall_output ERROR_VARIABLE reinstall_error)
if(NOT reinstall_status EQUAL 0)
    message(FATAL_ERROR "Bluetooth route reinstall failed:\n${reinstall_output}${reinstall_error}")
endif()
run_route(route_status)
if(NOT route_status MATCHES "[Tt]imeout")
    message(FATAL_ERROR
        "relocated Bluetooth route did not remain constructed (${route_status}):\n${route_status_log}")
endif()
message(STATUS "Relocated Bluetooth Settings route remained resident with host buses poisoned")
