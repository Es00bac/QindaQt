...
void DevelopmentInputProtocolTest::parsesAndDispatchesAllowlistedEvents()
{
    const auto payload = request(
        {QJsonObject{{QStringLiteral("type"), QStringLiteral("pointer-absolute")},
         ...
         QJsonObject{{QStringLiteral("type"), QStringLiteral("key")},
                     ...
                     {QStringLiteral("pressed"), false}},
         QJsonObject{{QStringLiteral("type"), QStringLiteral("key")},
                     {QStringLiteral("key"), QStringLiteral("up")},
                     {QStringLiteral("pressed"), true}},
         QJsonObject{{QStringLiteral("type"), QStringLiteral("key")},
    ...
    QCOMPARE(parsed->events.size(), 14);
    ...
    QCOMPARE(parsed->events.at(7).key, DevelopmentInputKey::Up);
    QVERIFY(parsed->events.at(7).pressed);
    QCOMPARE(parsed->events.at(8).key, DevelopmentInputKey::Down);
    QVERIFY(parsed->events.at(8).pressed);
    QCOMPARE(parsed->events.at(9).key, DevelopmentInputKey::Left);
    QVERIFY(parsed->events.at(9).pressed);
    QCOMPARE(parsed->events.at(10).key, DevelopmentInputKey::Right);
    QVERIFY(parsed->events.at(10).pressed);
    QCOMPARE(parsed->events.at(11).key, DevelopmentInputKey::Enter);
    QVERIFY(!parsed->events.at(11).pressed);
    QCOMPARE(parsed->events.at(12).button, DevelopmentInputButton::Left);
    QCOMPARE(parsed->events.at(13).button, DevelopmentInputButton::Right);
    ...
    QCOMPARE(reply.value(QStringLiteral("eventCount")).toInteger(), 14);
...
{
    // ADR-0191 rows roll windows up and down with real wheel notches: one
